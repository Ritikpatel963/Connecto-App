...
export const PersonCell = ({ name, subtitle, online, userId }: { name: unknown; subtitle?: unknown; online?: boolean, userId?: number | string }) => {
  const [realName, injectedPhone] = String(name || "Unknown").split(':::');
  const actualSubtitle = subtitle ?? (injectedPhone || undefined);
  const initials = realName.split(" ").map((part) => part[0]).slice(0, 2).join("");
  const content = (
    <div className="d-flex align-items-center gap-10 min-w-160-px">
      ...
      <div>
        <span className={`d-block fw-semibold ${userId ? 'text-primary-main' : 'text-primary-light'}`}>{realName}</span>
        {actualSubtitle !== undefined && actualSubtitle !== null && <span className="d-block text-xs text-secondary-light">{String(actualSubtitle)}</span>}
      </div>
    </div>
  );
